...
def add_vocabs(vocabs: List[Path], mappings: dict):
    ...
    for vocab in vocabs:
        r = httpx.post(
            "http://fuseki.surroundaustralia.com/dawe-vocabs/data",
    ...
    for f in Path(__file__).parent.parent.glob("vocabularies/*.ttl"):
        r2 = httpx.post(
            "http://fuseki.surroundaustralia.com/dawe-vocabs/update",
...
def get_graph_uri_for_vocab(vocab: Path) -> URIRef:
    ...
    have one and only one ConceptScheme per file and that the DAWE VocPrez installation uses the ConceptScheme URI
    as the Graph URI"""
...
if __name__ == "__main__":
    ...
    r = httpx.get("http://dawe.surroundaustralia.com/cache-reload")
